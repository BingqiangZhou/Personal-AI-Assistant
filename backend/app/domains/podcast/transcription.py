                ...
                async with aiofiles.open(destination, "wb") as f:
                    async for chunk in response.content.iter_chunked(self.chunk_size):
                        ...
                        if not first_chunk_logged:
                            preview = chunk[:200]
                            logger.info(
                                f"ℹ️ [Response Body Preview] First 200 bytes: {preview}"
                            )
        ...
        try:
            ...
            if os.path.exists(output_path):
                try:
                    ...
                    logger.warning(
                        f"⚠️ [CONVERT] Failed to remove partial output: {cleanup_error}"
                    )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            ...
            for chunk in locals().get("chunks", []):
                if os.path.exists(chunk.file_path):
                    try:
                        ...
                        logger.warning(
                            f"⚠️ [SPLIT] Failed to remove partial chunk: {cleanup_error}"
                        )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
class SiliconFlowTranscriber:
    ...
    async def transcribe_chunk(
        ...
        async with self.semaphore:  # 限制并发数
            ...
            for attempt in range(max_retries):
                ...
                try:
                    logger.info(
                        f"🎤 [CHUNK {chunk.index:03d}] Starting transcription (Attempt {attempt + 1}/{max_retries}), file={os.path.basename(chunk.file_path)}, size={chunk.file_size} bytes, model={model}"
                    )
                    ...
                    logger.info(
                        f"📡 [REQUEST] API Key (first 15 chars): {self.api_key[:15]}..."
                    )
                    logger.info(
                        f"📡 [REQUEST] API Key (last 5 chars): ...{self.api_key[-5:]}"
                    )
                    ...
                    async with self.session.post(self.api_url, data=data) as response:
                        ...
                        if response.status != 200:
                            ...
                            logger.error(
                                f"❌ [CHUNK {chunk.index:03d}] API error (Attempt {attempt + 1}): {response.status} - {error_text}"
                            )

                            # 记录本次尝试失败
                            if ai_repo and config_db_id:
                                try:
                                    await ai_repo.increment_usage(
                                        config_db_id, success=False
                                    )
                                    logger.debug(
                                        f"📊 [STATS] Recorded failure for chunk {chunk.index}, attempt {attempt + 1}"
                                    )
                                except Exception as stats_error:
                                    logger.warning(
                                        f"⚠️ [STATS] Failed to record failure stats: {stats_error}"
                                    )

                            if attempt < max_retries - 1:
                                delay = base_delay * (2**attempt)
                                logger.info(
                                    f"⏳ [CHUNK {chunk.index:03d}] Retrying in {delay}s..."
                                )
                        ...
                        chunk_elapsed = time.time() - chunk_start
                        logger.info(
                            f"✅ [CHUNK {chunk.index:03d}] Success! Got {transcript_len} chars in {chunk_elapsed:.2f}s"
                        )
                        ...
                        if ai_repo and config_db_id:
                            try:
                                await ai_repo.increment_usage(
                                    config_db_id, success=True
                                )
                                logger.debug(
                                    f"📊 [STATS] Recorded success for chunk {chunk.index}, attempt {attempt + 1}"
                                )
                            except Exception as stats_error:
                                logger.warning(
                                    f"⚠️ [STATS] Failed to record success stats: {stats_error}"
                                )
                    ...
                    chunk_elapsed = time.time() - chunk_start
                    logger.error(
                        f"❌ [CHUNK {chunk.index:03d}] Failed attempt {attempt + 1} after {chunk_elapsed:.2f}s: {str(e)}"
                    )
                    ...
                    if ai_repo and config_db_id:
                        try:
                            ...
                            logger.debug(
                                f"📊 [STATS] Recorded failure for chunk {chunk.index}, attempt {attempt + 1} (exception)"
                            )
                        except Exception as stats_error:
                            logger.warning(
                                f"⚠️ [STATS] Failed to record failure stats: {stats_error}"
    ...
    async def transcribe_chunks(
        ...
        tasks = [
            asyncio.create_task(
                self.transcribe_chunk(chunk, model, ai_repo, config_db_id)
            )
            for chunk in chunks
        ]
        ...
        for coro in asyncio.as_completed(tasks):
            try:
                ...
                logger.error(
                    f"Unexpected error in transcribe_chunks sequence: {str(e)}"
                )
        ...
        logger.info(
            f"Completed transcription of {success_count}/{len(chunks)} chunks in {duration:.2f}s"
        )
...
class PodcastTranscriptionService:
    ...
    async def create_transcription_task_record(
        ...
        if existing_task:
            ...
            if force:
                ...
                logger.info(
                    f"🗑️ [TRANSCRIPTION] Force mode: deleting existing task {existing_task.id}"
                )
                ...
                logger.warning(
                    f"⚠️ [TRANSCRIPTION] Task already exists with status {existing_task.status}"
                )
                ...
                logger.info(
                    f"🗑️ [TRANSCRIPTION] Removing failed/cancelled task {existing_task.id} before creating new one"
                )
                await self.db.delete(existing_task)
                await self.db.flush()
                await (
    ...
    async def execute_transcription_task(
        ...
        try:
            ...
            if task.status == TranscriptionStatus.CANCELLED:
                log_with_timestamp(
                    "WARNING",
                    f"⚠️ [SKIP] Task {task_id} was cancelled, skipping execution",
                    task_id,
                )
                return
            ...
            if os.path.exists(original_file) and os.path.getsize(original_file) > 0:
                ...
                log_with_timestamp(
                    "INFO",
                    f"⏭️ [STEP 1/6 DOWNLOAD] Skip! File already exists: {original_file} ({file_size / 1024 / 1024:.2f} MB)",
                    task_id,
                )
                log_with_timestamp(
                    "INFO",
                ...
                log_with_timestamp(
                    "INFO",
                    f"⏱️ [STEP 1/6 DOWNLOAD] Time taken: {download_time:.2f}s",
                    task_id,
                )
                ...
                if converted_size > 10240:  # 至少10KB
                    ...
                    try:
                        ...
                        if duration:
                            ...
                            log_with_timestamp(
                                "INFO",
                                f"⏭️ [STEP 2/6 CONVERT] Skip! Valid MP3 file already exists: {converted_file} ({converted_size / 1024 / 1024:.2f} MB, {duration}s)",
                                task_id,
                            )
                            log_with_timestamp(
                                "INFO",
                            ...
                            log_with_timestamp(
                                "WARNING",
                                f"⚠️ [STEP 2/6 CONVERT] File exists but invalid (no duration), re-converting: {converted_file}",
                                task_id,
                            )
                    except Exception as e:
                        log_with_timestamp(
                            "WARNING",
                            f"⚠️ [STEP 2/6 CONVERT] File exists but validation failed ({str(e)}), re-converting",
                            task_id,
                        )
                    else:
                        log_with_timestamp(
                            "WARNING",
                            f"⚠️ [STEP 2/6 CONVERT] File exists but too small ({converted_size} bytes), re-converting",
                            task_id,
                        )
                else:
                    log_with_timestamp(
            ...
            if not skip_conversion:
                log_with_timestamp(
                    "INFO", "🔄 [STEP 2/6 CONVERT] Starting MP3 conversion...", task_id
                )
                await self._update_task_progress_with_session(
            ...
            if os.path.exists(split_dir) and os.path.isdir(split_dir):
                ...
                if chunk_files:
                    log_with_timestamp(
                        "INFO",
                        f"⏭️ [STEP 3/6 SPLIT] Skip! Chunks already exist: {len(chunk_files)} files found",
                        task_id,
                    )
                    log_with_timestamp(
                    ...
                    for chunk_file in sorted(chunk_files):
                        ...
                        index = int(
                            chunk_file.replace("chunk_", "").replace(".mp3", "")
                        )
                    ...
                    log_with_timestamp(
                        "INFO",
                        f"✂️ [STEP 3/6 SPLIT] Starting audio split with chunk_size_mb={task.chunk_size_mb}...",
                        task_id,
                    )
                    await self._update_task_progress_with_session(
            ...
            if already_transcribed:
                log_with_timestamp(
                    "INFO",
                    f"⏭️ [STEP 4/6 TRANSCRIBE] Found {len(already_transcribed)} already transcribed chunks, skipping",
                    task_id,
                )
            ...
            if chunks_to_transcribe:
                ...
                log_with_timestamp(
                    "INFO",
                    f"⏱️ [STEP 4/6 TRANSCRIBE] Time taken: {transcription_time:.2f}s",
                    task_id,
                )
            else:
                # 所有片段都已转录
            ...
            # 步骤5：合并转录结果
            log_with_timestamp(
                "INFO", "🔗 [STEP 5/6 MERGE] Merging transcription results...", task_id
            )
            await self._update_task_progress_with_session(
            ...
            try:
                ...
                logger.warning(
                    f"⚠️ [STEP 6 SAVE] shutil.move failed ({e}), trying copy + delete"
                )
                ...
                try:
                    ...
                    logger.warning(
                        f"⚠️ [STEP 6 SAVE] Could not remove source file: {converted_file}"
                    )
            ...
            try:
                ...
                if final_status == TranscriptionStatus.COMPLETED:
                    ...
                    if os.path.exists(temp_episode_dir):
                        logger.info(
                            f"⏸️ [CLEANUP] Preserving temporary directory for task {task_id} (status={final_status}): {temp_episode_dir}"
                        )
            except Exception as e:
    ...
    async def _schedule_ai_summary(self, session: AsyncSession, task_id: int):
        ...
        try:
            # 获取转录任务
            log_with_timestamp(
                "INFO", f"🔍 [AI SUMMARY] Getting transcription task {task_id}", task_id
            )
            stmt = select(TranscriptionTask).where(TranscriptionTask.id == task_id)
            result = await session.execute(stmt)
            task = result.scalar_one_or_none()
            ...
            try:
                ...
                log_with_timestamp(
                    "WARNING",
                    f"⚠️ [AI SUMMARY] Failed to refresh task: {refresh_error}",
                    task_id,
                )
            ...
            log_with_timestamp(
                "ERROR",
                f"❌ [AI SUMMARY] Cannot update task {task_id} with error info in current transaction",
                task_id,
            )
